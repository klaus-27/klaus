...
async def start(client, message):
  if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
      ...
      InlineKeyboardButton('Mᴀɪɴ Cʜᴀɴɴᴇʟ', url=CHNL_LNK)
               ...
               [
                 InlineKeyboardButton(
                   'Hᴇʟᴘ', url=f"https://t.me/{temp.U_NAME}?start=help"),
               ]]
    ...
    await asyncio.sleep(
      2
    )  # 😢 https://github.com/GouthamSER/KuttuBot/blob/master/plugins/p_ttishow.py#L17 😬 wait a bit, before checking.
  ...
  if len(message.command) != 2:
    buttons = [[
      InlineKeyboardButton('Aᴅᴅ Mᴇ ᴛᴏ Yᴏᴜʀ Gʀᴏᴜᴘs',
  ...
  if AUTH_CHANNEL and not await is_subscribed(client, message):
    try:
      ...
      return
    btn = [[
      InlineKeyboardButton("Jᴏɪɴ Mᴀɪɴ Cʜᴀɴɴᴇʟ",
    ...
    await client.send_message(
      ...
      text="**Pʟᴇᴀsᴇ Jᴏɪɴ Mʏ Mᴀɪɴ Cʜᴀɴɴᴇʟ ᴛᴏ ᴜsᴇ ᴛʜɪs Bᴏᴛ!**",
  ...
  if data.split("-", 1)[0] == "BATCH":
    ...
    decoded = (base64.urlsafe_b64decode(b_string + "=" *
...
async def delete(bot, message):
  ...
  if reply and reply.media:
    ...
    await message.reply('Reply to file with /delete which you want to delete',
                        quote=True)
    return
